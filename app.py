...
st.caption("Instagram India 2025")

# -----------------------------------------------
# LOAD MODEL
# -----------------------------------------------
if not os.path.exists("model.pkl"):
    st.error("model.pkl not found. Please train the model in Colab first.")
    st.stop()
...
# -----------------------------------------------
st.header("3. Trend Direction: Latest vs Old")
...
trend_df = pd.DataFrame({
    "Hashtag":        all_tags,
    "Old Avg":        old_al.values.round(0).astype(int),
    "Latest Avg":     lat_al.values.round(0).astype(int),
    "Change %":       pct.values.round(1),
    "Trend":          pct.values
}).assign(Trend=lambda x: x['Trend'].apply(get_trend))
trend_df = trend_df.sort_values("Change %", ascending=False).reset_index(drop=True)
...
with tab1:
    st.dataframe(rising[['Hashtag','Old Avg','Latest Avg','Change %','Trend']],
                 use_container_width=True, hide_index=True)
with tab2:
    st.dataframe(stable[['Hashtag','Old Avg','Latest Avg','Change %','Trend']],
                 use_container_width=True, hide_index=True)
with tab3:
    st.dataframe(falling[['Hashtag','Old Avg','Latest Avg','Change %','Trend']],
                 use_container_width=True, hide_index=True)
...
# -----------------------------------------------
st.header("4. Predicted Engagement (Next 7 Days)")

future_input = pd.DataFrame({
    'day_number':  np.arange(df['day_number'].max() + 1, df['day_number'].max() + 8),
    'moving_avg':  [float(df['moving_avg'].iloc[-1])] * 7,
    'growth_rate': [float(df['growth_rate'].iloc[-1])] * 7
})
...
# Top 5 hashtags monthly
st.subheader("Top 5 Hashtags - Monthly Trend")
...
for col in monthly.columns:
    ax3.plot(monthly.index, monthly[col], marker='o',
             markersize=3, label=col, linewidth=1.5)
ax3.set_xlabel("Month")
ax3.set_ylabel("Avg Engagement")
ax3.set_title("Monthly Engagement - Top 5 Hashtags")
